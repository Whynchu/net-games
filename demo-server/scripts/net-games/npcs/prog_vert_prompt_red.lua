...
Talk.npc({
  ...
  on_interact = function(player_id, _bot_id, bot_name)
    Talk.vert_menu(player_id, bot_name, {
      ...
      options = { count = 40, prefix = "Red Option ", pad = 2, exit_text = "Exit", exit_id = "exit" },
      ...
      -- Use preset SFX set by key (still overrideable)
      sfx = "card_desc",

      -- You can override behavior if desired (still short)
      flow = "prog_prompt",
      layout = "prog_prompt",
    })
  end,
